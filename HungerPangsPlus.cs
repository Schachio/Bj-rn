...
namespace Schachio.HungerPangsPlus
{
    ...
    public sealed class HungerPangsPlusPlugin : BaseUnityPlugin
    {
        ...
        private float _nextEat, _nextRefill, _nextHarvest, _autoEatPausedUntil;
        ...
        private void Update()
        {
            ...
            if(_toggleShortcut.Value.IsDown())
            {
                ...
                Config.Save();
                try { p.Message(MessageHud.MessageType.Center, PluginName+": Automatik "+(_masterEnabled.Value?"AN":"AUS")); } catch { }
            }
        ...
        private void HandleFoodSlotMouseClick()
        {
            ...
            for(int i=0;i<icons.Length;i++)
            {
                ...
                if(RectTransformUtility.RectangleContainsScreenPoint(rect,mouse,cam)){OnFoodSlotClicked(i);return;}
            }
        }
        ...
        private void OnFoodSlotClicked(int slot)
        {
            ...
            {
                ItemDrop.ItemData returned=active.m_item.Clone();returned.m_stack=1;
                if(!inv.CanAddItem(returned,1)){try { p.Message(MessageHud.MessageType.Center,PluginName+": Inventar voll - Essen bleibt aktiv"); } catch { }return;}
            ...
            _autoEatPausedUntil=Time.time+pause;_nextEat=_autoEatPausedUntil;
        ...
        private static float FoodScore(ItemDrop.ItemData i){return IsFood(i)?i.m_shared.m_food+i.m_shared.m_foodStamina+i.m_shared.m_foodEitr:0f;}
        private static bool IsPreparedFood(ItemDrop.ItemData i)
        {
            if(!IsFood(i)) return false;
            string n=(i.m_shared.m_name??"").ToLowerInvariant();string p=(i.m_dropPrefab!=null?i.m_dropPrefab.name:"").ToLowerInvariant();string s=n+" "+p;
        ...
        private static float BaseSelectionScore(ItemDrop.ItemData i){return FoodScore(i)+(IsPreparedFood(i)?10000f:0f);}
        private static bool CanEatSilently(Player p,ItemDrop.ItemData c)
        {
            if(!IsFood(c)) return false;List<Player.Food> f=p.GetFoods();if(f==null) return true;
            foreach(Player.Food a in f){if(a==null||a.m_item==null||a.m_item.m_shared==null) continue;if(a.m_item.m_shared.m_name==c.m_shared.m_name) return a.CanEatAgain();}
            return f.Count<3;
        }
        private void TryAutoEat(Player p)
        {
            ...
            {try { if(p.ConsumeItem(inv,f,false)) return; }catch(Exception e) { Logger.LogDebug("Auto-eat skipped item: "+e.Message); }}
        }
        private bool IsAtBase(Vector3 pos)
        {
            ...
            {if(s==null||s.gameObject==null) continue;string o=s.gameObject.name??"",n=s.m_name??"";bool w=o.IndexOf("workbench",StringComparison.OrdinalIgnoreCase)>=0||n.IndexOf("workbench",StringComparison.OrdinalIgnoreCase)>=0||n.IndexOf("$piece_workbench",StringComparison.OrdinalIgnoreCase)>=0;if(w&&Vector3.Distance(pos,s.transform.position)<=r) return true;}
            return false;
        }
        private List<Container> GetNearbyContainers(Player p)
        {
            float r=Mathf.Max(1f,_containerRadius.Value);
            return UnityEngine.Object.FindObjectsOfType<Container>().Where(c=>c!=null&&c.gameObject!=null&&Vector3.Distance(p.transform.position,c.transform.position)<=r).OrderBy(c=>Vector3.Distance(p.transform.position,c.transform.position)).ToList();
        }
        private bool CanAccessContainer(Container c,long playerId){if(!IsSafeStorage(c)) return false;try { return c.CheckAccess(playerId); }catch { return false; }}
        private void TryRefill(Player p)
        {
            Inventory target=p.GetInventory();if(target==null) return;long id=p.GetPlayerID();List<Container> cs=GetNearbyContainers(p);List<ItemDrop.ItemData> foods=new List<ItemDrop.ItemData>();
            foreach(Container c in cs){if(!CanAccessContainer(c,id)) continue;Inventory src=c.GetInventory();if(src!=null) foods.AddRange(src.GetAllItems().Where(IsFood));}
            int types=Mathf.Clamp(_foodTypesToStock.Value,1,3);
            List<string> names=foods.GroupBy(x=>x.m_shared.m_name).Select(g=>g.OrderByDescending(BaseSelectionScore).First()).OrderByDescending(BaseSelectionScore).Take(types).Select(x=>x.m_shared.m_name).ToList();
            foreach(Player.Food a in p.GetFoods()) if(a!=null&&a.m_item!=null&&a.m_item.m_shared!=null&&!names.Contains(a.m_item.m_shared.m_name)) names.Insert(0,a.m_item.m_shared.m_name);
            names=names.Distinct().Take(types).ToList();
            foreach(string name in names)
            {
                int need=10-CountFood(target,name);if(need<=0) continue;
                foreach(Container c in cs)
                {
                    if(need<=0) break;if(!CanAccessContainer(c,id)) continue;Inventory src=c.GetInventory();if(src==null) continue;
                    foreach(ItemDrop.ItemData item in new List<ItemDrop.ItemData>(src.GetAllItems()))
                    {if(need<=0) break;if(!IsFood(item)||item.m_shared.m_name!=name) continue;int amount=Math.Min(need,item.m_stack);ItemDrop.ItemData copy=item.Clone();copy.m_stack=amount;if(!target.CanAddItem(copy,amount)) return;if(target.AddItem(copy)){src.RemoveItem(item,amount);need-=amount;}}
                }
            }
        }
        private static void PutSelectedFoodOnHotbar(Inventory inv)
        {
            if(inv==null) return;List<ItemDrop.ItemData> foods=inv.GetAllItems().Where(IsFood).OrderByDescending(BaseSelectionScore).Take(3).ToList();
            foreach(ItemDrop.ItemData food in foods){if(food.m_gridPos.y==0) continue;for(int x=0;x<8;x++){if(inv.GetItemAt(x,0)!=null) continue;food.m_gridPos=new Vector2i(x,0);inv.Changed();break;}}
        }
        private static int CountFood(Inventory i,string n){int c=0;foreach(ItemDrop.ItemData x in i.GetAllItems()) if(x!=null&&x.m_shared!=null&&x.m_shared.m_name==n)c+=x.m_stack;return c;}
        ...
        {
            Inventory inv=p.GetInventory();if(inv==null) return;float r=Mathf.Max(.5f,_harvestRadius.Value);
            foreach(Pickable x in UnityEngine.Object.FindObjectsOfType<Pickable>())
            {if(x==null||x.gameObject==null||Vector3.Distance(p.transform.position,x.transform.position)>r||!x.CanBePicked()) continue;GameObject prefab=x.m_itemPrefab;if(prefab==null) continue;ItemDrop d=prefab.GetComponent<ItemDrop>();if(d==null||!IsFood(d.m_itemData)||!inv.CanAddItem(prefab,1)) continue;try { x.Interact(p,false,false); }catch(Exception e) { Logger.LogDebug("Auto-harvest failed: "+e.Message); }}
        }
    }
}
